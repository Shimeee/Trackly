...
export function SignUpScreen() {
  ...
  const onSubmit = async () => {
    ...
    setLoading(true);
    try {
      const account = await signUpWithPassword(email.trim(), password, firstName.trim(), lastName.trim());
      completeOnboarding();
  ...
  return (
    <Screen>
      ...
      <Text style={[textType.body, { color: colors.textSecondary }]}>{t('signUp.subtitle')}</Text>

      <View style={{ gap: 8 }}>
        <Text style={[textType.caption, { color: colors.textSecondary }]}>{t('signUp.firstName')}</Text>
        <TextInput
          value={firstName}
      ...
      <View style={{ gap: 8 }}>
        <Text style={[textType.caption, { color: colors.textSecondary }]}>{t('signUp.lastName')}</Text>
        <TextInput
          value={lastName}
      ...
      <View style={{ gap: 8 }}>
        <Text style={[textType.caption, { color: colors.textSecondary }]}>{t('signUp.email')}</Text>
        <TextInput
          value={email}
      ...
      <View style={{ gap: 8 }}>
        <Text style={[textType.caption, { color: colors.textSecondary }]}>{t('signUp.password')}</Text>
        <TextInput
          value={password}
      ...
      <Button label={t('signUp.submit')} disabled={!canSubmit} loading={loading} onPress={onSubmit} />
    </Screen>
  );
}
